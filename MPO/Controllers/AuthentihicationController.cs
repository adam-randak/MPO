...
namespace MPO.Controllers
{
    [Route("api/[controller]")]
    [ApiController]
    public class AuthentihicationController : ControllerBase
    {
        private readonly UserManager<User> userManager;
        private readonly RoleManager<IdentityRole> roleManager;
        private readonly IConfiguration _configuration;

        public AuthentihicationController(UserManager<User> userManager, RoleManager<IdentityRole> roleManager, IConfiguration configuration)
        {
        ...
        public async Task<IActionResult> RegisterAdmin([FromBody] RegisterModel model)
        {
            ...
            if (await roleManager.RoleExistsAsync(UsersRole.Admin))
            {
                await userManager.AddToRoleAsync(user, UsersRole.Admin);
            }
            return Ok(new AuthenticationResponse { Status = "Success", Message = "User created successfully!" });
        }
        ...
        public async Task<IActionResult> Login([FromBody] LoginModel model)
        {
            ...
            if (user != null && await userManager.CheckPasswordAsync(user, model.Password))
            {
                ...
                var authSigningKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_configuration["JWT: Secret key ffdsfds"]));
